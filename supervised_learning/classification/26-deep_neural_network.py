...
class DeepNeuralNetwork:
    ...
    def train(self, X_train, Y_train, iterations=500, graph=True):
        """
        Train the deep neural network

        Args:
            X_train: numpy.ndarray - input data of shape (nx, m)
            Y_train: numpy.ndarray - labels of shape (1, m)
            iterations: int - number of training iterations
            graph: bool - whether to display the training cost graph

        Returns:
            A: numpy.ndarray - activated outputs of shape (1, m)
            costs: list - list of costs after each iteration
        """
        costs = []
